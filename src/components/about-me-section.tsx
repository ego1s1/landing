...
const nvimComponents = {
  ...
  h3: ({ children }: { children: React.ReactNode }) => (
    <h3 className="text-[var(--th-cyan)] font-bold text-xs mt-2 mb-1 flex items-center gap-1.5">
      <span className="text-[var(--th-green)]">▸</span>
      {children}
    </h3>
  ),
  p: ({ children }: { children: React.ReactNode }) => (
    <p className="text-[var(--th-text)] text-xs leading-5 mb-1.5">{children}</p>
  ),
  ...
  blockquote: ({ children }: { children: React.ReactNode }) => (
    <blockquote className="border-l-2 border-[var(--th-purple)] pl-3 py-0.5 bg-[var(--th-surface-alt)]/15 rounded-r text-[var(--th-text-muted)] italic text-xs my-1.5">
      {children}
    </blockquote>
  ),
  code: ({ children }: { children: React.ReactNode }) => (
    <code className="bg-[var(--th-surface-alt)] text-[var(--th-green)] px-1 py-0.5 rounded text-[10px] border border-[var(--th-border-subtle)]/40">
      {children}
    </code>
  ),
};
    ...
    >
      ...
      <div className="bg-[var(--th-bg)] overflow-hidden">
        ...
        <div className="flex items-center gap-1.5 bg-[var(--th-surface)] px-2 py-1 text-[11px] font-mono select-none">
          ...
          <span className="text-[var(--th-yellow)] text-[10px]">●</span>
        ...
        <div className="flex min-h-[150px]">
          ...
          <div className="hidden sm:flex flex-col items-end select-none bg-[var(--th-surface)]/30 border-r border-[var(--th-border-subtle)]/30 px-2 py-2.5 text-[10px] leading-5 font-mono text-[var(--th-text-dim)] min-w-[36px]">
            <span className="text-[var(--th-text-dim)]/60 text-[9px] leading-5 mb-0.5">⎻</span>
            {Array.from({ length: lineCount }).map((_, i) => {
              ...
              return (
                <span
                  key={n}
                  className={`leading-5 flex items-center gap-1 justify-end w-full ${isBlank ? "text-[var(--th-border-subtle)]/30" : isCursor ? "text-[var(--th-yellow)] font-bold" : "text-[var(--th-text-dim)]"}`}
                >
            ...
            {Array.from({ length: fillerCount }).map((_, i) => (
              <span key={`f-${i}`} className="leading-5 text-[var(--th-border-subtle)]/30 text-[10px]">
                ~
              </span>
            ))}
          </div>
          ...
          <div className="flex-1 relative py-2.5 px-3 sm:px-4 overflow-hidden">
            ...
            <div className="absolute left-0 right-0 top-[2.55rem] h-5 bg-[var(--th-surface-alt)]/25 border-y border-[var(--th-border-subtle)]/15 pointer-events-none" />
            {/* sign column indicator */}
            <div className="absolute left-0 top-[2.55rem] bottom-auto w-0.5 h-5 bg-[var(--th-yellow)]/80 hidden sm:block" />
            ...
            <div className="absolute right-2 top-2 hidden lg:block text-[10px] font-mono bg-[var(--th-surface-alt)] text-[var(--th-text-dim)] border border-[var(--th-border-subtle)] px-1.5 py-0.5 rounded opacity-60">
              󰈙 friendly
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center bg-[var(--th-surface-alt)]/80 text-[10px] font-mono select-none">
          ...
          <span className="ml-auto flex items-center gap-1 px-2 py-1 text-[var(--th-text-muted)]">
            ...
            <span>markdown</span>
            <span className="hidden sm:inline-flex items-center gap-1 ml-1 bg-[var(--th-bg)] border border-[var(--th-border-subtle)] px-1 py-0.5 rounded text-[9px]">
